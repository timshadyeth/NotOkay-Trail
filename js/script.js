...
var game = {
  totalDays: 0,
  daysLeft: 50,
  imgArray: ["url(img/trail.jpeg)", "url(img/trail2.jpg)", "url(img/trail3.jpg)"],
  imgArrayIndex: 0
};
...
var checkpoints = ["Las Vegas", "the Big Blue River", "Fort Bridger", "the Snake River"];
...
function fates(roll, rivOrTrail) {
  ...
  if (rivOrTrail === "trail") {
    if (roll <= 10) {
      ...
      var diseaseNames = ["dysentery", "cholera", "typhoid", "measles", "diphbearia", "scurvy"];
      ...
      $("#randomEventMessage").text(caravan.party[charIndex].name + " broke a foot while running from Clarke.");
      caravan.party[charIndex].health -= 50;
    } else if (roll<=18 && caravan.food > 0){
      $("#randomEventMessage").text("Everyone shunned " + caravan.party[charIndex].name+" after dropping a bushel of fish!");
      caravan.food -= 50;
    } else if (roll<=21){
      $("#randomEventMessage").text("There was a Viper in " +  caravan.party[charIndex].name + "'s boot. " + caravan.party[charIndex].name + " contracted a disease.");
      caravan.party[charIndex].diseases += 1;
    } else if (roll >= 98) {
      caravan.food += 50;
      $("#randomEventMessage").text("Your caravan came across a field of ripe, delicious bearies.");
    } else if (roll >= 95) {
      caravan.medicine += 1;
      $("#randomEventMessage").text("A generous, traveling apothebearie named Mary has gifted you 1 medicine.");
    } else if (roll >= 92) {
      caravan.party.forEach(function (element) {
        element.healthGain();
      });
      $("#randomEventMessage").text("You found a not-too-hot hot nft! Your party feels more rested.");
    } else {
      $("#event").html("You have traveled a day and are one  step closer to Mad Toast World.");
    }
    ...
    if (roll <= 10) {
      ...
      caravan.food -= amount;
      $("#randomEventMessage").text("The river was rough and " + caravan.party[charIndex].name + " dropped " + amount + " fish in the river.");
    } else if (roll <= 25) {
      caravan.party[charIndex].diseases += 1;
      ...
      $("#event").text("Your party successfully crossed the river. Onward to Mad Toast World.")
      return;
    }
  } else {
    console.log("ERRORRR");
...
function talk() {
  ...
  if(talkRoll === 0) {
    $("#event").text("Howdy, Y'all! My name is Timshady, I'm the town blacksmith. It's a pleasure making your aquaintance.");
  }else if(talkRoll === 1) {
    $("#event").text("Why, hello there! My name is TheVoiceOfCash, I'm the Mayor in these here parts. It's a pleasure making your aquaintance.");
  }else if(talkRoll === 2) {
    $("#event").text("GIMME YA LOOTS YA DARN BEARS. THIS HERE'S A STICK UP.");
  }else if(talkRoll === 3) {
...
function gameChecker() {
  if (game.daysLeft === 0) {  // GAME OVER WIN
    ...
    $("#checkPoint").html("WINNER! WINNER! BEAR DINNER! Only " + left + " of your party has survived.");
...
function hunt() {
  ...
  $("#event").html("Everyone in your party gathered "+meatGained+" fish!");
  foodLoss();
  caravan.party.forEach(function (element) {
    element.healthLoss();
  });
...
function updateStats() {
  ...
  var foodString = "";
  if (caravan.food <= 0) {
    foodString = "<span id='foodZero'>fish: 0</span>";
  } else {
    foodString = "fish: " + caravan.food;
  }
...
$(function() {
  $("form#createParty").submit(function(event) {
    ...
    var autoNames = ["TimShady", "VoiceOfCash", "Lisa", "Clarke", "LitBunny", "Trems", "JoyRide", "Mustafa Shakir", "Viper", "CryptoC0wboy", "Jzone", "BRENT", "Askinvest", "JusCoffeedrinker", "open24hrs", "GutterBabe", "ThatLemon", "Vipress", "melibug"];
    caravan.party.forEach(function(member) {
      if (!member.name) {
  ...
  $(".heal").click(function() {
    $(".imgHeader").css("background-image", "url(img/dinosaurtrail.jpeg)");
    medicine();
    updateStats();
  });
